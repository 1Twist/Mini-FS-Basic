...
/*──────────────── helpers de perm ─────*/
static inline int dir_has_perm(const Dir *d,uint16_t bit)
{
    if (auth_uid()==0) return 1;                /* root tudo-pode  */
    uint16_t cls = (auth_uid()==d->owner)        ? (d->perms>>6)&7 :
                   (auth_gid()==d->group)        ? (d->perms>>3)&7 :
                                                   d->perms     &7 ;
    return (cls & bit)!=0;
}
...
static Dir *_dir_new(const char *name,Dir *parent)
{
    ...
    /* segurança padrão: herdamos máscara-padrão do usuário
       garantindo que diretório tenha ‘x’ onde houver ‘r’.        */
    User *u   = auth_get_user_by_uid(auth_uid());
    uint16_t base = u ? u->dflt_perms : 0666;
    uint16_t addx = ((base & 0444) >> 2);   /* transforma r-bits em x */
    d->perms = (base | addx) & 0777;
